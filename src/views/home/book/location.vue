			...
			<div class="top-content">
				<el-table :data="tableData" stripe style="width: 100%; margin-bottom: 20px" row-key="id" default-expand-all>
					...
					<el-table-column prop="location" label="地点">
						<template #default="scope">
							<span v-for="item in scope.row.locations" :key="item.id" style="margin-right: 8px">
								<el-tag
									><el-link @click="handleSelect('location-detail', item.id)">{{ item.name }}</el-link></el-tag
								>
							</span>
						</template>
					</el-table-column>
...
import {
	queryByTypeCategoryApi,
	queryByAreaCategoryApi,
	queryByElementApi,
	queryByLevelRealmApi
} from "@/api/management/book/book-location"
...
const handleTabChange = (name: TabPaneName) => {
	if (name == 1) {
		queryByTypeCategoryApiData()
	} else if (name == 2) {
		queryByAreaCategoryApiData()
	} else if (name == 3) {
...
const queryByTypeCategoryApiData = () => {
	loading.value = true
	queryByTypeCategoryApi({
...
const queryByAreaCategoryApiData = () => {
	loading.value = true
	queryByAreaCategoryApi({
		bookId: route.params.bookId
	})

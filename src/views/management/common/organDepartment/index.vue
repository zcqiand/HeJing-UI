<template>
	<div class="app-container">
		<el-card v-loading="loading" shadow="never">
			<el-row :gutter="10">
				<el-col :span="4">
					<el-menu @select="handleOrganSelect" class="sub-menu-wrapper" :default-active="organId">
						<el-form ref="searchOrganFormRef" :inline="true" :model="searchOrganData" class="sub-search-wrapper">
							<el-input
								v-model="searchOrganData.name"
								placeholder="输入关键字过滤"
								:prefix-icon="Search"
								@input="handleOrganSearch"
							/>
						</el-form>
						<el-menu-item v-for="o in organData" :key="o.id" :index="o.id">
							<div>{{ o.name.substring(0, 12) }}<span v-if="o.name.length > 12">...</span></div>
						</el-menu-item>
					</el-menu>
				</el-col>
				<el-col :span="20" v-if="organData.length > 0 && organId != undefined">
		...
		<edit v-if="dialogVisible" ref="editRef" :organId="organId" @success="handleSaveSuccess"></edit>
	</div>
</template>
...
import { deleteApi, batchDeleteApi, queryApi } from "@/api/management/common/organDepartment"
...
onMounted(() => {
	if (route.query.organId !== undefined) {
		organId.value = route.query.organId as string
	}
	queryOrganData()
})
...
watch(
	() => route.query.organId,
	newOrganId => {
		organId.value = newOrganId as string
		queryTableData()
	}
)
...
//#region 菜单
//查询
const handleOrganSearch = () => {
	queryOrganData()
}

//选择
const handleOrganSelect = (key: string | undefined) => {
	router.push(`?organId=${key}`)
}

//获取清单
const searchOrganFormRef = ref<FormInstance | null>(null)
const searchOrganData = reactive({
	name: ""
})
const organData = ref<any[]>([])
const queryOrganData = () => {
	queryOrganApi({
		pageIndex: 1,
		pageSize: 2000,
		name: searchOrganData.name || undefined
	})
		.then((res: any) => {
			organData.value = res.data.items
			if (res.data.items !== undefined && res.data.items.length > 0) {
				if (organId.value === undefined) {
					organId.value = res.data.items[0].id
				}
				queryTableData()
			}
		})
...
const queryTableData = () => {
	if (organId.value !== undefined && organId.value.length > 0) {
		loading.value = true
		queryApi({
...
@/api/management/common/organs@/api/management/common/organDepartment
